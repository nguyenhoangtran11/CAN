...
import com.nht.provisioning.device_scanner.BleScanner;
import com.nht.provisioning.device_scanner.WiFiScanner;
import com.nht.provisioning.listeners.BleScanListener;
import com.nht.provisioning.listeners.QRCodeScanListener;
...
public class CanProvisionManager {
     ...
     * @return Returns
     */
    public static CanProvisionManager getInstance(Context context) {

        if (provision == null) {
            provision = new CanProvisionManager(context);
        }
        return provision;
    }

    private CanProvisionManager(Context context) {
        this.context = context;
        handler = new Handler();
    }

    /**
     * This method is used to get CanDevice object with given transport and security.
     *
     * @param transportType Transport type.
     * @param securityType  Security type.
     * @return Returns CanDevice.
     */
    public CanDevice createESPDevice(CanConstants.TransportType transportType, CanConstants.SecurityType securityType) {

        canDevice = new CanDevice(context, transportType, securityType);
        return canDevice;
    }
    ...
    private void processQrCode(String qrCode, QRCodeScanListener qrCodeScanListener) {
        try {
            ...
            int security = jsonObject.optInt("security", CanConstants.SecurityType.SECURITY_2.ordinal());
            ...
            CanConstants.TransportType transportType = null;
            CanConstants.SecurityType securityType = null;

            if (!TextUtils.isEmpty(transport)) {

                if (transport.equalsIgnoreCase("softap")) {

                    transportType = CanConstants.TransportType.TRANSPORT_SOFTAP;

                } else if (transport.equalsIgnoreCase("ble")) {

                    transportType = CanConstants.TransportType.TRANSPORT_BLE;
            ...
            canDevice = new CanDevice(context, transportType, securityType);
            canDevice.setDeviceName(deviceName);
            canDevice.setProofOfPossession(pop);
            canDevice.setUserName(userName);

            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q && transportType.equals(CanConstants.TransportType.TRANSPORT_SOFTAP)) {
                WiFiAccessPoint wiFiDevice = new WiFiAccessPoint();
                wiFiDevice.setWifiName(deviceName);
                wiFiDevice.setPassword(password);
                canDevice.setWifiDevice(wiFiDevice);
                qrCodeScanListener.deviceDetected(canDevice);
            } else {
                isDeviceAvailable(canDevice, password, qrCodeScanListener);
            }
    ...
    /**
     * This method will check given CanDevice is available in scanning or not.
     * It will give callback to app with CanDevice object if device is available in scanning.
     *
     * @param device             CanDevice.
     * @param password           Password for Wi-Fi device.
     * @param qrCodeScanListener QRCodeScanListener to give callbacks to app.
     */
    private void isDeviceAvailable(final CanDevice device, String password, QRCodeScanListener qrCodeScanListener) {
    ...
    private CanConstants.SecurityType setSecurityType(int security) {
        switch (security) {
            case 0:
                return CanConstants.SecurityType.SECURITY_0;
            case 1:
                return CanConstants.SecurityType.SECURITY_1;
            case 2:
            default:
                return CanConstants.SecurityType.SECURITY_2;
        }
    }
    ...
    class SearchDeviceTask implements Runnable {
        ...
        SearchDeviceTask(CanDevice device1, String password1, QRCodeScanListener listener1) {
            searchCnt++;
            device = device1;
            listener = listener1;
            password = password1;
        ...
        public void run() {
            ...
            if (device.getTransportType().equals(CanConstants.TransportType.TRANSPORT_BLE)) {
