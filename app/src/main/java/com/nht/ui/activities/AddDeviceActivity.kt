...
class AddDeviceActivity : AppCompatActivity() {
    ...
    private var canDevice: CanDevice? = null
    ...
    override fun onResume() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q && canDevice != null && canDevice!!.transportType == CanConstants.TransportType.TRANSPORT_SOFTAP) {
            val ssid = wifiSsid
            Log.d(TAG, "Currently connected WiFi SSID : $ssid")
            Log.d(TAG, "Device Name  : ${canDevice!!.deviceName}")
            if (!TextUtils.isEmpty(ssid) && ssid != canDevice!!.deviceName) {
    ...
    fun onEvent(event: DeviceConnectionEvent) {
        ...
        when (event.eventType) {
            CanConstants.EVENT_DEVICE_CONNECTED -> {
            ...
            if (ActivityCompat.checkSelfPermission(
                ...
                if (isPlayServicesAvailable) {
                    ...
                    cameraProviderFuture.addListener({
                        try {
                            // Start QR code scanning using CanProvisionManager
    ...
    private val qrCodeScanListener = object : QRCodeScanListener {
        ...
        override fun deviceDetected(device: CanDevice) {
            ...
            runOnUiThread {
                ...
                when (deviceType) {
                    AppConstants.DEVICE_TYPE_BLE -> {
                        if (canDevice != null && canDevice!!.transportType == CanConstants.TransportType.TRANSPORT_SOFTAP) {
                    ...
                    AppConstants.DEVICE_TYPE_SOFTAP -> {
                        if (canDevice != null && canDevice!!.transportType == CanConstants.TransportType.TRANSPORT_BLE) {
                            ...
                            if (canDevice!!.transportType == CanConstants.TransportType.TRANSPORT_SOFTAP && Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                                val wifiManager =
    ...
    private fun askForManualDeviceConnection() {
        ...
        builder.setPositiveButton(R.string.btn_yes) { dialog, _ ->
            dialog.dismiss()
            canDevice?.let { device ->
                when (device.securityType) {
                    CanConstants.SecurityType.SECURITY_0 -> goToWiFiProvisionLandingActivity(
                        AppConstants.SEC_TYPE_0
                    )

                    CanConstants.SecurityType.SECURITY_1 -> goToWiFiProvisionLandingActivity(
                        AppConstants.SEC_TYPE_1
                    )

                    CanConstants.SecurityType.SECURITY_2 -> goToWiFiProvisionLandingActivity(
    ...
    private fun startProvisioningFlow() {
        ...
        when (deviceType) {
            AppConstants.DEVICE_TYPE_BLE -> {
                if (isSecure) {
                    provisionManager.createESPDevice(
                        CanConstants.TransportType.TRANSPORT_BLE,
            ...
            AppConstants.DEVICE_TYPE_SOFTAP -> {
                if (isSecure) {
                    provisionManager.createESPDevice(
                        CanConstants.TransportType.TRANSPORT_SOFTAP,
                        CanConstants.SecurityType.SECURITY_2
                    )
                } else {
                    provisionManager.createESPDevice(
        ...
        try {
            ...
            if (provInfo.has("sec_ver")) {
                ...
                if (securityType == AppConstants.SEC_TYPE_2) {
                    securityType = AppConstants.SEC_TYPE_1
                    provisionManager.espDevice?.securityType = CanConstants.SecurityType.SECURITY_1
                }
            }
    ...
    private fun processDeviceCapabilities() {
        val deviceCaps = canDevice?.deviceCapabilities
        when {
